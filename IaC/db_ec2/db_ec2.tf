...
#db server용 보안그룹 인바운드 규칙 생성 - 모든 트래픽 허용
resource "aws_vpc_security_group_ingress_rule" "db_in_all" {
...
#db server용 보안그룹 아웃바운드 규칙 생성 - 모든 트래픽 허용
...
resource "aws_instance" "db_server_instance" {
  ...
  subnet_id                 = var.public_subnet_id_02 #퍼블릭 서브넷 id
